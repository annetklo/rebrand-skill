# Fase 1: Landscape audit (touchpoints + concurrentie-kaart)

Method file for Fase 1 of /rebrand. Runs only after the Fase 0 gate (creative brief approved). Output is one HTML gate page, opened as a local review surface, that feeds the visual territories (see `references/brand_strategy.md`). The Fase 1 gate itself (territory choice) happens after territories are built; this file produces the evidence those territories stand on.

Ground rules for this phase:

- Zero image-generation cost. Everything here is screenshots, `curl` downloads, and researcher subagents. No image-generation model (Gemini or otherwise) is invoked in Fase 1.
- All output lands in the single engagement directory from SKILL.md (Output locations), one per engagement: `<engagement-map>/{yymmdd}-rebrand-{merk}/` in your workspace (e.g. `<engagement-map>/260831-rebrand-mr/`), with its shared `assets/` subfolder for screenshots and downloaded logo files. Fase 1 does not open a new directory; it writes into the directory Fase 0 created, so the gate pages of all phases can reference each other and the brief relatively. Never write into the live brand-style folder of the existing identity; the current brand assets stay untouched until Fase 5 migration.
...
Before looking outward, inventory the brand as it exists today. This wall is the "before" evidence and becomes the migration checklist in Fase 5.
...
| Offertes / voorstellen | newest issued quote or proposal PDF in the document archive | `pdftoppm -png -f 1 -l 1` on page 1 |
| Decks | most recent deck export (PDF) | `pdftoppm` cover + one content slide |
| LinkedIn | profile header + 2 recent posts with visuals | screenshot via an authenticated browser session (bijv. een browser-automation MCP) |
| Nieuwsbrief | latest newsletter edition | screenshot of header + first scroll |
...
- Save as `assets/touchpoint_<naam>.png`.

Per touchpoint, write exactly one Dutch line labeled **Wat schuurt:**. It must be observable in the screenshot and name the artifact. Good: "Wat schuurt: de raket staat hier in een andere coral (#F26E5A) dan op de site (#ED7059)." Bad: "Wat schuurt: het voelt gedateerd." If nothing genuinely schuurt, write "Consistent, geen frictie." and move on; do not invent friction to seem thorough.
...
**Ring 1, direct competitors (target 8-12).** Brands a prospect would shortlist next to this one. Worked example, Mission Relearn: NL bureaus and solo consultants selling AI-geletterdheid training, AI-literacy workshops, AI-adoptie-begeleiding to government, education, and enterprise. Candidate sources, in this order:
1. The creative brief (Fase 0 names who the brand must convince and against whom).
2. Optional project knowledge (bijv. een clients-bestand als je dat hebt): competitors that surfaced in real deals.
3. Web search: for the MR example "AI geletterdheid training", "AI training bureau overheid", "AI literacy consultancy Nederland", plus vendors visible in the ecosystems the brand operates in (bijv. een onderwijsconsortium of nationaal onderwijsprogramma waar de klant aan deelneemt).

**Ring 2, sector neighbours (target 5-8).** Not direct rivals, but brands the buyer sees in the same week. For the MR example: NL EdTech products, learning platforms, generalist consultancies with an AI practice, government program identities (nationale onderwijsprogramma's), trainingsbureaus outside AI.

**Ring 3, aspirational and anti-reference (target 3-5).** Taken from Discovery ronde 2 (admired brands and anti-references). Include at least one anti-reference on purpose: it calibrates the far end of the map.
...
> Vraag: "Dit is de scan-lijst: [n] directe concurrenten, [n] sector-buren, [n] referentiemerken. Klopt deze lijst?"
> Opties: "Lijst klopt, start de scan" / "Er missen merken (ik noem ze)" / "Haal er merken af"
...
3. core_color: the dominant brand hue as a band (primair blauw / blauwpaars
   gradient / groen / oranje-coral / rood / zwart-grijs / multicolor). Do not
...
7. tone_words: 3 words the brand itself uses on its homepage, verbatim
   (e.g. "groei", "leren", "verbinden", "empower").
...
not observable, write "onbekend" instead of guessing. Do not print any
...
- **NL onderwijs/training.** Institutional layer (ministeries, nationale programma's, stichtingen): fresh accessible palettes, connection/community symbolism, gradients "voor energie en dynamiek". Commercial trainingsbureaus: warm-but-corporate blue or orange, person photos, rounded sans, tone words "groei", "leren", "verbinden". Residual: crests and serifs at traditional universities. Worked-example implication for Mission Relearn: coral sits off the dominant hue band of this market, which makes it an existing differentiator to protect, not a candidate for replacement. The audit must confirm this against the corpus: count how many scanned brands use a warm red-orange.
...
The blacklist below goes on the gate page verbatim and is copied forward into the Fase 2 concept notes and the Fase 3 design brief for the form-engine run (the logo-generator skill, github.com/op7418/logo-generator-skill, if installed; else direct SVG generation). Any concept or variant containing one of these gets flagged automatically; it may only survive with a written justification tied to a named brief attribute (Duolingo-style subversion counts, lazy use does not).

> **Verboden zonder expliciete motivatie:** brein, neurale nodes/synapsen, circuit-sporen, hexagon, robot, uil, afstudeerhoed, open boek, potlood, gloeilamp, swoosh, radiale gradient-cirkel met opening in het midden, generieke groei-pijl omhoog, handdruk, puzzelstukjes, kennis-boom.
...
The map is visual, not strategic. Business-positioning axes (price, segment, service model) belong in the Fase 0 brief; this map plots what a buyer can SEE. Four rules:

1. **Visual and observable.** Both poles must be judgeable from a homepage screenshot plus logo alone. The test: could two people independently place a brand on the axis within one step of each other, using only the captures? "Innovatief ↔ traditioneel" fails this test (it is an opinion); "geometrisch ↔ organisch" passes.
2. **Orthogonal.** If the plotted corpus forms a rough diagonal, the axes correlate and you are measuring one thing twice ("premium ↔ budget" x "hoogwaardig ↔ eenvoudig" is the classic double). Reject the pair and pick again. Do this rejection visibly: the gate page notes which pair was tried and dropped.
3. **Contrasting poles, Dutch labels.** Each pole is a real opposite, phrased so the client and an outside reader read it the same way.
...
- "warm ↔ koel" (palet) x "geometrisch ↔ organisch" (vormtaal)
- "speels ↔ formeel" (toon) x "abstract ↔ letterlijk" (merkteken)
...
1. **Leeg**: genuinely empty in the scanned corpus (point at the map).
2. **Gewild**: desirable to the target buyer named in the brief (point at the brief attribute it serves).
3. **Geloofwaardig**: ownable and credible for this brand with its history and assets (point at an existing asset or proof).

Empty because nobody wants it is a trap, not an opportunity. If no quadrant passes all three, say so on the gate page and propose the least-crowded credible position instead; a forced white-space claim poisons Fase 2.
...
- Fame: does the existing audience already meet this element everywhere? (In the MR worked example: coral appears in all output, site, offertes, decks, LinkedIn. That is seed fame.)

Score at minimum the brand's accent color, its mark, its wordmark treatment, and its heading typeface as brand voice (for MR: the coral, the raket mark, the wordmark, Cal Sans). Standing rule: never abandon an asset with existing fame for novelty. High-fame assets are constraints on the territories, and that is a feature; it is what makes "evolutie" cheaper than "revolutie".
...
1. **Touchpoint-wand.** All touchpoint screenshots at equal width, each with its "Wat schuurt:" line and the logo-variant/accent-hex/font findings.
2. **Brandscape-wand.** Every corpus logo in an identical 160px tile, grouped by ring, brand name + hue band as caption.
3. **Cluster-samenvatting.** The tallies (hue band, form family, type class) as small bar counts, the named cluster phrase if the 70% threshold is hit, and the residual/dominant/emergent table with the Keller keep/break decision per code.
4. **2x2-kaart.** Inline SVG, Dutch pole labels, ring-coded markers, brand-now marker in the brand's accent color, the rejected axis pair named in a footnote.
5. **Cliche-blacklist.** The verboden-lijst plus corpus-specific additions, flagged as "gaat mee naar fase 2 en 3".
6. **Witruimte-statement.** Exactly one Dutch sentence, this template: "De witruimte voor [merk] is [visuele positie in as-termen], omdat [doelgroep uit de brief] daar [gevoel/betekenis] zoekt en geen van de [n] gescande merken daar staat." Below it, the three validity tests as pass/fail rows with one line of evidence each, and the Fame x Uniqueness scores of the current assets.
7. **Volgende stap.** One line: "Deze kaart voedt de 2-3 territories (fase 1-gate). Kies hieronder."

Open the gate page as a local review surface (de HTML in de browser):

```
open "<engagement-map>/{yymmdd}-rebrand-{merk}/{YYMMDD}_Landscape-audit.html"
```

Then one AskUserQuestion call (this is a review checkpoint, not the Fase 1 gate; the gate follows the territories):

> Vraag: "Klopt deze kaart met jouw beeld van de markt?"
> Opties: "Ja, bouw de territories op deze witruimte" / "De assen kloppen niet, probeer een ander paar" / "Er missen merken, scan er meer"
...
- Every brand has all recorded fields filled or explicitly "onbekend"; no guessed hex codes.
- Every touchpoint has a screenshot AND a "Wat schuurt:" line (or "Consistent, geen frictie.").
