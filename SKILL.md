...
1. **Strategy before form.** No visual work starts before the creative brief is approved at
   ...
   takes one session and makes every later phase cheaper, then offer Fase 0 as the starting
...
2. **One carrying idea per concept.** Every concept must be expressible in a single sentence
   ("het merkteken is de baan die je opnieuw instapt"). If a concept needs a paragraph to
...
the user, asked via AskUserQuestion in Dutch. Record every gate decision in the
...
### Fase 0 · Discovery: het vraagprotocol · Gate: brief akkoord
...
1. Run the four question rounds from the question bank, every question via AskUserQuestion
   ...
   order: (1) ambitie & scope, including the single most
   important question of the whole engagement: evolutie of revolutie; (2) positionering,
   including personality sliders and admired brands plus anti-references with the why;
   (3) beeldmerk: what the current mark means, whether the story still holds, whether the
   sign must work standalone at favicon/avatar size; (4) woordmerk & systeem: name
...
3. Ask Wolff Olins' two butterfly questions somewhere in rounds 1 or 2, verbatim in Dutch:
   "Wat maakt jullie echt bijzonder?" and "Wat heeft de wereld van jullie nodig?" The
   overlap of the two answers is the raw material for the brand idea.
4. Frame the evolutie/revolutie question with its real criterion: revolution is justified
   ...
   instead of resetting it. The forcing question: "lossen we een echt strategisch probleem
   op, of reageren we op merk-verveling?"
5. Build the creative brief from `assets/brief_template.html`. Include an explicit equity
   inventory: wat houden we, wat schrappen we, wat mag vrij bewegen. Record answers
...
scope decision (evolutie of revolutie), a decision log entry.

**Gate.** Open the brief on the review surface, collect feedback, revise (maximum two
revision rounds, this skill's hard bound), then ask: "Is deze brief akkoord als
fundament voor alles wat volgt?" Options: "Akkoord, door naar research" / "Nog één
revisieronde" / "Stop hier, brief is de oplevering". No visual work until this gate passes.

### Fase 1 · Research & audit · Gate: territory-keuze
...
1. **Touchpoint-audit**: inventory where the brand actually lives. For Mission Relearn:
   missionrelearn.com, offertes en voorstellen, decks, LinkedIn, nieuwsbrief, dashboard,
   Academy. Screenshot each, note what jars against the brief. For a client: ask for their
   top-5 touchpoints (already captured in Fase 0 round 4) and audit those.
2. **Landscape-scan**: launch researcher subagents (web research) on the sector's visual
   ...
   consultancies, overheidsleveranciers. Output: a 2x2 map of "waar staat iedereen, waar is
   ruimte", with named competitors placed on the axes.
3. **Visual territories**: build 2 or 3 moodboard directions, each a coherent world of
   typography, color, and form language with a name ("warme wetenschap", "helder
   instituut", "de vonk" are the shape of it, derive real ones from the brief). This is the
...
AskUserQuestion: "Welk territory
wordt de basis voor de concepten?" with one option per territory plus "Combineer elementen
(licht toe via Other)". Log the choice.

### Fase 2 · Concept: één dragend idee · Gate: concept-keuze
...
1. Per concept, write a concept note containing: the idea in one sentence; why it fits the
   brief (quote the brief's own words back); how it plays through beeldmerk, woordmerk, and
   language; rough sketches (SVG or described); and the kill-criteria: the observable
   conditions under which this concept has failed. Kill-criteria keep the fase-3 iteration
   honest, because "ik vind het niet mooi" is not a kill-criterion but "het teken is
   onleesbaar op 16px" is.
2. Make the concepts span the evolutie-revolutie spectrum decided in Fase 0. Pentagram
...
**Gate.** Open the page, collect feedback, then: "Welk concept draagt de rebrand?" Options per concept
plus "Hybride (motiveer via Other)". A hybrid choice requires the user to name which idea
...
### Fase 3 · Design-exploratie (de logo-generator draait hier) · Gate: richting-keuze

**Read first**: `references/wordmark_typography.md` and `references/color_system.md`. The
beeldmerk track additionally uses the logo-generator's own
...
beeldmerk, woordmerk, kleur. This skill's own rule for the gate: present 4 to 6 defensible
...
**Steps.**
1. **Beeldmerk track, via the form engine.** If the `logo-generator` skill
...
2. **Woordmerk track.** A full track, not an afterthought, and never a typed line: a
...
3. **Kleur track.** Build the palette around the core color per
...
4. Present all three tracks on one exploration page, reusing the design language of
   ...
   systeem-context-mock: a fragment of the client's real design language (page header,
   kicker, card) with the candidate in place, before any grid of tiles. The deciding
   question for a mark is not "is het mooi op een tegel" but "hoort het merk er ineens
   bij"; that reframing came verbatim from a client and unlocked a stalled engagement.
5. Iterate through at most two annotation rounds on the review surface, then freeze. If
   ...
   to further tweaking is "dan is het concept niet goed genoeg, terug naar fase 2", said
   ...
   (see "Taste recovery en smaak-gates").
...
**Deliverables.** Concept-driven exploration page; a chosen beeldmerk, woordmerk, and
palette.

**Gate.** "Welke combinatie van beeldmerk, woordmerk en palet gaat door naar het systeem?"
Freeze the choice, log it.

### Fase 4 · Systeem & stress-test · Gate: go/no-go per drager
...
2. Build the fixed mockup set from the reference file on real carriers: favicon 16px,
   LinkedIn-avatar en -banner, deck-cover, offerte-header, site-header,
   e-mailhandtekening, sticker.
3. Run degradation tests: zwart-wit kopie, klein op een beamer, dark/light behavior.
4. Anything that fails a degradation test goes back into a bounded fix loop within the
   frozen direction; it does not reopen the fase-3 gate.

**Deliverables.** Mockup page per application, the definitive lockup set, presented on a
gate page for annotation.

**Gate.** The per-drager go/no-go gate from `references/applications_stresstest.md` Step 5
closes this phase: every drager gets its own recorded go or no-go (batched at most 4
questions per AskUserQuestion call). Fase 5 may only migrate a drager that has its own
recorded go.

### Fase 5 · Delivery & migratie · Gate: livegang per drager
...
2. **Guidelines**: build the brand-guidelines page from `assets/guidelines_template.html`
   ...
   approval only, update your optional project knowledge (bijv. een brand-bestand,
   brand-reference-pagina of design-assets-skill, als je die hebt) so the new identity
   becomes the working default.
3. **Migratieplan**: an ordered replacement list (site, LinkedIn, templates, decks,
   offertes) with a go/no-go per carrier. Include a launch-timing note: which carriers
   switch together, which can trail.

**Deliverables.** Version folder with assets, guidelines page, updated brain knowledge,
migration checklist.

**Gate.** Execute migration per drager against the recorded go-list from the Fase 4 gate;
do not re-ask the 9 questions. Re-ask only a drager whose status changed since that gate
...
("nog niet", "lelijk"), the fix is upstream, not more variants. In a real run, four failed
design rounds were recovered in a single round with this protocol.

- **Three taste-directive questions** (via AskUserQuestion, in Dutch), asked the moment a
  second consecutive round is rejected:
  1. "Wat stootte je af in de laatste ronde?" Options: "beschadiging" / "te veel uitleg" /
     "te hard" / "gewoon niet mooi".
  2. "Welke eerdere artefacten raakten wél iets?" Name concrete candidates as options,
     including rounds from BEFORE the current concept. In the run, the client named the
     very first exploration ("de planeet met de uitsparing") and a recolored wordmark,
     which killed the active concept lineage entirely.
  3. "Moet het teken het verhaal dragen, of mag het puur passen?" Options: "puur passen" /
     "zichtbaar verhaal" / "ertussenin".
- **Lineage kills, not variant kills.** A gate answer like "begin opnieuw, terug tot
  voordat je X voorstelde" kills a whole concept LINEAGE (the concept and all its
...
- **Light rationales when the client asks for taste.** Full concept-notities at a smaak
  gate are concept theater with a cost: when the question on the table is "mooi", give
  each candidate a one-line rationale instead. Meaning may legitimately land on
  "ertussenin", a light reference, if the client explicitly chooses that.
- **Calibration rounds.** Per surviving lane, offer 3 calibrations of ONE variable each
  (bijv. gatgrootte, ringhoek, fase-diepte). Clients choose confidently between
  calibrations where they stall on concepts; in the run, two calibration rounds went from
  three surviving lanes to one chosen mark. Calibration rounds count toward the fase-3
  round bound.

## Fase 3 integration: the logo-generator as form engine
...
- **Output routing**: its SVG variants and showcase HTML land in the current
  ...
  the Fase 5 version folder, never into the skill's own directory or the existing
  brand-style assets.
...
- **Questions**: all interactive questions go through AskUserQuestion, maximum 4 questions
  per call, 2 to 4 options each, Dutch, and the user can always answer free-form via
  Other. Never dump the question bank as a wall of text. One round of questions per turn,
  with a short reflection between rounds.
...
- **One decision-owner**: Pentagram routes all feedback through a single internal point
  ...
  Fase 0 round 1 who owns the decisions, and accept gate choices only from that person's
...
- **Decisions are logged**: every gate outcome goes into the engagement's decision log
  ...
  sessions later.
- **Language**: instructions and internal notes in English; everything the client
  sees (briefs, gate questions, guidelines, migration plans) in natural Dutch, unless the
  client asks for English.
...
- **Working pages** (briefs, audits, territories, concepts, explorations, mockups,
  ...
  (`<engagement-map>/{yymmdd}-{slug}/`), opened on the local review surface (de HTML in de browser).
  **One directory for the whole engagement**, e.g. `<engagement-map>/260831-rebrand-mr/`,
  ...
  comes from filename prefixes (`{YYMMDD}_{fase}_...`), never from separate directories:
...
- **Client deliverables** follow the document-output rules (Dutch, PDF for external
  sharing).

## Running a partial engagement

Fase 0 + 1 alone is a valid, sellable engagement: a signed creative brief plus an audit and
...
it. Other valid cuts: Fase 0 through 2 (strategy and concept, design later or elsewhere),
or Fase 3 through 5 on top of an existing brief (verify the brief meets the Fase 0 bar
first: carrying idea, equity inventory, evolutie/revolutie decision; if not, run a
compressed Fase 0 to fill the gaps). Every partial run still ends at its phase gate with a
