...
The skill is written in English: the question bank, templates, and process instructions. Everything the client sees (gate questions, discovery questions, briefs, guidelines) runs in the client's language, translated on the fly. The worked example engagement ran in Dutch.

## The six phases

| Phase | Name | What happens | Gate (client decides) |
|---|---|---|---|
| 0 | Discovery | Four structured question rounds (~20-28 questions), the evolution/revolution decision, equity inventory, creative brief | "Is this brief approved as the foundation?" |
| 1 | Research & audit | Touchpoint audit, competitive landscape scan, 2x2 map, 2-3 visual territories | "Which territory becomes the basis?" |
| 2 | Concept | 2-3 concepts, each with one carrying idea in one sentence, plus explicit kill-criteria | "Which concept carries the rebrand?" |
| 3 | Design exploration | Three parallel tracks: mark (icon geometry), wordmark (letterform surgery on real font outlines), color (palette with computed WCAG ratios) | "Which combination goes through?" |
...
| 5 | Delivery & migration | Versioned asset export, brand guidelines, ordered migration plan | Go-live per carrier |

Each phase ends in a gate page the client reviews and annotates. Open the gate page as a local review surface (simply the HTML in the browser); the client's decision is asked explicitly, in the client's language, and logged. Nothing is "done" without a recorded decision. Partial engagements are valid: Phase 0+1 alone (brief plus audit) is a sellable milestone.

## What makes it different

- **A taste-recovery protocol.** When a client rejects rounds repeatedly ("not yet", "ugly"), the fix is upstream, not more variants. The skill switches to three taste-directive questions: what repelled, which earlier artifacts did land (named concretely, including rounds from before the current concept), and whether the mark must carry the story or may simply fit. In the worked engagement this recovered the direction in one round after four failed design rounds. A gate answer like "start over, go back to before you proposed X" kills a whole concept lineage, not one variant; killed work is archived, not deleted, because the client's favorites can resurrect it under a new reading.
- **Judging in system context.** The deciding question for a mark is not "does it look good on a tile" but "does the brand suddenly belong": every gate page opens with a mock fragment of the client's real design language (page header, kicker, card) with the candidate in place.
- **Kill-criteria per concept.** "I don't like it" is not a kill-criterion; "the sign is illegible at 16px" is. Every concept ships with the observable conditions under which it has failed, which keeps iteration honest.
...
- Optional: the **logo-generator skill** by op7418 (github.com/op7418/logo-generator-skill) as the form engine in Phase 3; if it is not installed, the skill generates SVG variants directly from the geometry recipes in its references.
...
The skill uses **Mission Relearn** (an AI-literacy consultancy in the Netherlands) as its worked example throughout: the discovery answers, the territory names, the geometry of the exploration lanes, and the taste-recovery round all come from a real engagement run in late 2026. That engagement ran in Dutch: the skill's question bank and templates are English, and all client-facing output (gate questions, discovery questions, brief and guidelines text) is translated on the fly into the client's language. That is deliberate. A process skill with a real brand threaded through it is easier to follow than one full of placeholders; substitute your own brand's brief and the same machinery applies.
